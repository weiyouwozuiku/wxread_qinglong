...
def getenv(id :str):
    data=QLAPI.getEnvs({"searchValue": id})
    return data['data'][0]['value']

def updateEnv(id :str,value):
    data= QLAPI.getEnvs({"searchValue": id})['data'][0]
    data['value']=value
    QLAPI.updateEnv({"env":data})

# 阅读次数 默认120次/60分钟
READ_NUM = int(getenv('READ_NUM') or 120)
READ_NUM+=random.Random().randint(0,20)
